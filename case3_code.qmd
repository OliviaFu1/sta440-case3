...
# Hierarchical LMM
library(lme4)
lme.out<-lmer(obsTTmodified~(1|region_category)+DISPATCH.PRIORITY.NAME,data=x)
summary(lme.out)
...
# lmm model
df_changed <- df_model |> filter(change_flag)
...
# Hierarchical LMM 
library(lme4)
lme.out<-lmer(obsTTmodified~(1|region_category)+DISPATCH.PRIORITY.NAME,data=x2)
summary(lme.out)
...
## LMM Modeling Part #3 using dispatch rule 1
```{r}
library(dplyr)
library(tidyr)
library(stringr)
...
model_df <- x |>
  dplyr::transmute(
    row_val = dplyr::row_number(),region_category, DISPATCH.PRIORITY.NAME,
    dplyr::across(matches("^S\\d+_ETT_DISPATCH$"))
  ) |>
  tidyr::pivot_longer(
  ...
  dplyr::mutate(
    Scenario        = stringr::str_remove(Scenario, "_ETT_DISPATCH$"),
...
lmm_model_final <- lmer(
  ETT_min ~ Scenario * region_category + DISPATCH.PRIORITY.NAME +
    (1 | row_val),
  data = model_df
)
