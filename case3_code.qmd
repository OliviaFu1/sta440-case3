...
# logistic model
model_logit <- glm(change_flag ~ region_category, 
                   data = model_df, 
                   family = binomial)
summary(model_logit)

# lmm model
df_changed <- model_df |> filter(change_flag)
model_lmm <- lmer(ETT_sec ~ Scenario + (1|row_val), data = df_changed)
summary(model_lmm)
